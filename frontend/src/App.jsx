...
export default function App() {
  ...
  async function submitText(question) {
    ...
    setResult(null);
    setInput("");
    setResult(await askText(question));
    setLoading(false);
  }
  ...
  async function submitAudio() {
    setLoading(true);
    setResult(null);
    setResult(await askAudio(new Blob(chunks.current, { type: "audio/webm" })));
    setLoading(false);
  }
  ...
  async function onMic() {
    ...
    try {
      ...
      current.onstop = submitAudio;
      current.start();
      recorder.current = current;
      setRecording(true);
    } catch {
      setResult({ status: "error", answer: "Microphone access was denied.", timing: {} });
    }
  }
  ...
  return (
    <div className="app-shell">
      ...
      <div className="app-body">
        {view === askView ? (
          <AskPage {...{ result, loading, submitText, recording, onMic, input, setInput, onReset }} />
        ) : (
          <InfoPage view={view} />
        )}
      </div>
